...
function ShortcutItemGroup({ category }: { category: string }) {
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-sm text-[var(--app-text-muted)]">{getCategoryName(category)}</h3>
...
function ShortcutItem({ action, shortcutKey }: { action: string; shortcutKey: string }) {
  return (
    <div className="flex items-center justify-between rounded border border-[var(--app-border)] px-2 py-1">
