...
const faqs = [
  ...
  },
  {
    ...
    answer: (
      <span>
        本工具在「设置」的「功能设置」中提供了鼠标穿透开关。开启鼠标穿透时，窗口对鼠标隐身，可以点击窗口背后的内容。窗口右下角会显示当前状态。
      </span>
    )
  },
  {
...
export function FAQ() {
  return (
    <HelpSection Icon={BookOpen} title="常见问题">
      {faqs.map((faq, index) => (
        <div key={index} className="border border-[var(--app-border)] rounded-lg p-4">
          <h3 className="font-semibold mb-2">{faq.question}</h3>
          <p className="text-sm text-[var(--app-text)]">{faq.answer}</p>
        </div>
      ))}
    </HelpSection>
  )
