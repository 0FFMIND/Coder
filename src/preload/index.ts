...
const api = {
  ...
  triggerShortcutAction: (action: string) => ipcRenderer.invoke('triggerShortcutAction', action),
  getPromptPresets: () => ipcRenderer.invoke('getPromptPresets'),
  setWindowCollapsed: (collapsed: boolean) => ipcRenderer.invoke('setWindowCollapsed', collapsed),
  setWindowHeight: (height: number) => ipcRenderer.invoke('setWindowHeight', height),
  endRendererResize: () => ipcRenderer.invoke('endRendererResize'),
  onWindowCollapsedChanged: (callback: (collapsed: boolean) => void) => {
    ipcRenderer.on('window-collapsed-changed', (_event, collapsed) => callback(collapsed))
  },
  removeWindowCollapsedChangedListener: () => {
    ipcRenderer.removeAllListeners('window-collapsed-changed')
  },
  ...
  onSolutionClear: (callback: (resetScreenshots: boolean) => void) => {
    ipcRenderer.on('solution-clear', (_event, resetScreenshots) => callback(resetScreenshots))
  },
  ...
  // Navigate to coder page (requested by main process before executing coder actions)
  onNavigateToCoder: (callback: () => void) => {
    ipcRenderer.on('navigate-to-coder', callback)
  },
  removeNavigateToCoderListener: () => {
    ipcRenderer.removeAllListeners('navigate-to-coder')
  },
  ...
  onStopTranscriptionInput: (callback: () => void) => {
    ipcRenderer.on('stop-transcription-input', callback)
  },
  removeStopTranscriptionInputListener: () => {
    ipcRenderer.removeAllListeners('stop-transcription-input')
  },
  ...
  onAutoThemeChanged: (callback: (theme: 'dark' | 'light') => void) => {
    ipcRenderer.on('auto-theme-changed', (_event, theme) => callback(theme))
  },
