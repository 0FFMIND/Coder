...
### Local Electron Restart With Logs

When restarting the local Electron dev app on Windows, write all new dev output to
`electron-dev.log` in the project root.

1. Kill only processes that belong to this project:

```powershell
Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -match 'interview-coder-cn-main|electron-vite dev|npm run dev|electron.exe' } |
  Select-Object ProcessId,ParentProcessId,Name,CommandLine

$processIds = @(/* paste the project ProcessId values here */)
foreach ($procId in $processIds) {
  $proc = Get-Process -Id $procId -ErrorAction SilentlyContinue
  if ($proc) { Stop-Process -Id $procId -Force }
}
```
...
$log = Join-Path $project 'electron-dev.log'
Add-Content -LiteralPath $log -Value "`n===== restart $(Get-Date -Format o) =====`n" -Encoding UTF8
Start-Process -FilePath cmd.exe -ArgumentList @(
  '/d',
  '/s',
  '/c',
  "cd /d `"$project`" && set ELECTRON_RUN_AS_NODE=&& set ELECTRON_FORCE_IS_PACKAGED=&& npm run dev >> electron-dev.log 2>&1"
) -WindowStyle Hidden
```

Important: clear `ELECTRON_RUN_AS_NODE` before starting. If it is set to `1`, Electron runs
like Node and the main process can crash with `electron.app` being `undefined`.
